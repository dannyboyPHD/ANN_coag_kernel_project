...
target_scaling = [{'log'}];
target         = scaling_simple(target(:),output_min_max,target_scaling);
...
if strcmp(Language,'fortran')
    ...
    out_mm = scale_minmax(output_min_max,target_scaling);
    f = declare_local_array_fortran(f,'output_min_max',size(output_min_max),...
                            reshape(out_mm,[size(output_min_max,1)*size(output_min_max,2),1]),'%.8e');
    ...
    % Layers
      f = Layer_fortran(f,"! input scaling - end","x_hid_1","w1","x_scaled","b1",1,false,activation_fun);
    ...
    for k = 2:network.numLayers
        if k == network.numLayers
            ...
            w = "w"+num2str(k);
            b = "b"+num2str(k);
            x = "x_hid_"+num2str(k-1);
            f = Layer_fortran(f,"! Layer "+num2str(k-1)+" - end",x_out,w,x,b,k,true,activation_fun);
        else
            x_out = "x_hid_"+num2str(k);
            w = "w"+num2str(k);
            b = "b"+num2str(k);
            x = "x_hid_1"+num2str(k-1);
            f = Layer_fortran(f,"! Layer "+num2str(k-1)+" - end",x_out,w,x,b,k,false,activation_fun);
        end
    end
    
    %output scaling
    f = output_scaling_fortran(f,"! Layer "+num2str(k)+" - end",network.name,x_out,"output_min_max",output_min_max,target_scaling);

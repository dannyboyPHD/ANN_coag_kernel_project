...
def read_in_net(net_file):
    ...
    return weight_label, weights, bias_label,bias,no_inputs,no_layers,arch,net_name
...
def write_hc_fcn(f,fcn_name,weight_label,w,bias_label,b,no_inputs,no_layers,arch,scaling,act):

    # f.write(first_line(fcn_name,weight_label,bias_label,no_layers))
    f.write('double precision function '+fcn_name+'(data_in)\n')
    f.write('! no_inputs = '+str(no_inputs)+', [ v1, v2, T/vis, v1/mfp, v2/mfp,T ]\n')
    f.write('! no_outputs = '+str(arch[-1])+', [beta_av, corr_factor]\n' )
    f.write('! no_layers = '+str(no_layers)+', arch = ['+str(no_inputs)+'],'+str(arch[:])+'\n')
    # f.write('double precison, dimension('+str(no_inputs)+'), intent(in) :: data_in\n')
    f.write(douprec_dim_in(str(no_inputs),'data_in'))
    f.write(douprec_dim_dummy(str(no_inputs),'data_inputs'))
# weights
    ...
    for _ in range(no_layers):
        ...
        if _ ==0:
            for i in range(off,off+arch[_]*no_in-1 ):
                f.write(str(w[i])+',&\n')
            ...
            for i in range(off,off+arch[_]*arch[_+1]-1 ):
                f.write(str(w[i])+',&\n')
            ...
            for i in range(off , off + arch[-2]*arch[-1]-1):
                # print(arch[_-1])
                # print('i',i)
                f.write(str(w[i])+',&\n')
            f.write(str(w[i+1]))
    ...
    for _ in range(no_layers):
        dim = str(arch[_])
        f.write(douprec_dim_in_hc(dim,bias_label[_]))
        f.write('= (/ &\n')
        
        for i in range(off,off+arch[_] -1):
            f.write(str(b[i])+',&\n')
        f.write(str(b[i+1])+'/)\n')
        off = arch[_]
    ...
    output_mins = [ float(g.readline()) for _ in range(arch[-1])] 
    scaling_maxs = [ float(g.readline()) for _ in range(no_in)] 
    output_maxs = [ float(g.readline()) for _ in range(arch[-1])] 
    ...
    f.write('=(/ ')
    for _ in range(no_in-1):
        if scaling[_] == 'lin':
            f.write(str(scaling_mins[_])+',&\n')
        elif scaling[_] == 'log':
            f.write('log10('+str(scaling_mins[_])+'),&\n')
  
    if scaling[no_in-1] == 'lin':
        f.write(str(scaling_mins[-1])+'/)\n')
    elif scaling[no_in] == 'log':
        f.write('log10('+str(scaling_mins[-1])+') /)\n')

    f.write('double precision, dimension('+str(no_in)+') :: max_in')
    f.write('=(/ ')
    for _ in range(no_in-1):
        if scaling[_] == 'lin':
            f.write(str(scaling_maxs[_])+',&\n')
        elif scaling[_] == 'log':
            f.write('log10('+str(scaling_maxs[_])+'),&\n')
    
    if scaling[no_in-1] == 'lin':
        f.write(str(scaling_maxs[-1])+'/)\n')
    elif scaling[no_in] == 'log':
        f.write('log10('+str(scaling_maxs[-1])+') /)\n')
    ...
    for _ in range(arch[-1]-1):
        if scaling[no_in+_] == 'lin':
            f.write(str(output_mins[_])+',&\n')
        elif scaling[no_in+_] == 'log':
            f.write('log10('+str(output_mins[_])+'),&\n')
    if scaling[no_in+arch[-1]-1] == 'lin':
        f.write(str(output_mins[_+1])+' /)\n')
    elif scaling[no_in + arch[-1]-1] == 'log':
        f.write('log10('+str(output_mins[_+1])+') /)\n')
    ...
    for _ in range(arch[-1]-1):
        if scaling[no_in+_] == 'lin':
            f.write(str(output_maxs[_])+',&\n')
        elif scaling[no_in+_] == 'log':
            f.write('log10('+str(output_maxs[_])+'),&\n')

    if scaling[no_in+arch[-1]-1] == 'lin':
        f.write(str(output_maxs[_+1])+'/) \n')
    elif scaling[no_in + arch[-1]-1] == 'log':
        f.write('log10('+str(output_maxs[_+1])+') /)\n')
...
#   module based 'mod' or hard coded 'hc'
    code_structure = 'hc'

   

    w_l,w,b_l,b,no_in,no_lay,arch,net_name = read_in_net('net_name')
    print(b)

    if code_structure == 'mod': 
        ...
        with open('./input_files/'+net_name+'_bias.txt') as k:
            no_b = 0
            for l in range(no_lay):
                no_b = arch[l] + no_b
            
            b = [float(k.readline()) for _ in range(no_b)]
            # print(len(w))
            k.close()
    ...
    if code_structure == 'mod':
        ...
        write_hc_fcn(f,'beta_ann',w_l,w,b_l,b,no_in,no_lay,arch,scaling,act)
